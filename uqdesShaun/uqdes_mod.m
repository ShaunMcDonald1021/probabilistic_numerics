function [uensemble, t, logfntime, m_deriv_svec] ...
    = uqdes_mod(sspan,nsolves,N,kernel,lambda,alpha,odefn,u0,theta,...
    use_gpu, use_sparse, tgrid)

%% uqdes_mod.m 
%
% Modification of Oksana's uqdes.m code to be able to use sparse
% arrays, GPU's, or both.
% Most of the code (including comments) is directly ported
...
% use_gpu: a logical indicating whether or not to use GPU
% use_sparse: a logical indicating whether or not to use sparse structure
% for covariance matrices. Only sensible for uniform kernel
% 
% OUTPUTS
...
if use_gpu % This suffices to make sure all calculations are on GPU
    sspan = gpuArray(sspan);
    u0 = gpuArray(u0);
...
if use_sparse
    C_deriv_ssmat = sparse(RR(t,t,lambda,sspan(1),sspan(2)))/alpha;
    C_state_ssmat = sparse(QQ(t,t,lambda,sspan(1),sspan(2)))/alpha;
    C_cross1_ssmat = sparse(QR(t,t,lambda,sspan(1),sspan(2)))/alpha;
else
...
for n = tinds(1:end-1)'
    ...
    if ~use_sparse
        ...
        m_state_svec(endind,:,:) = m_state_svec(endind,:,:) + ...
            (full(kron(reshape(f_diff(1,:,:),size(f_diff, [2 3])), ...
            C_cross1_ssmat(endind,n))), [size(C_cross1_ssmat(endind,n),1) size(f_diff, [2 3])]);
        m_deriv_svec(ind,:,:) = m_deriv_svec(ind,:,:) + ...
            reshape(full(kron(reshape(f_diff(1,:,:),size(f_diff, [2 3])), ...
            C_deriv_ssmat(ind,n))), [size(C_deriv_ssmat(ind,n),1) size(f_diff, [2 3])]);
    end
    ...
    % Slight difference from original uqdes convergence check:
    % we must sum over ALL indices of f_diff to check a scalar.
    % The original check didn't work, as it would give a non-scalar
    % gpuArray which can't be converted into a logical
    if sum(f_diff >= 1e10 | isnan(f_diff) | isinf(f_diff), 'all') > 0
...
% A more efficient approach may be to have this function output gpuArray's,
% then only gather the stuff we need in each application.
% However, doing it this way allows for more honest benchmarking
if use_gpu
