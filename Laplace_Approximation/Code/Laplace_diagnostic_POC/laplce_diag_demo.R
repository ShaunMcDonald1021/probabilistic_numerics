...
for(i in seq_along(samp_sizes)){
  imp_list_1970[[i]] = imp_sampler_parallel(samp_sizes[i], 5, diag_1970$logf, diag_1970$T_mat, diag_1970$mode)
...
for(i in 1:100){
  ...
  small_checkcon = checkConsistency(fit_1970$obj, par = fit$opt$par, n = 100)
  ...
  large_checkcon = checkConsistency(fit_1970$obj, par = fit$opt$par, n = 1000)
